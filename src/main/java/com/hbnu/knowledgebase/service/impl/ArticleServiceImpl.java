...
//    }
    ...
    @Override
    public void publishNewVersion(Article article) {
        // 如果 articleNum 为空，说明是新文章
        if (article.getArticleNum() == null) {
            Integer maxNum = articleMapper.selectMaxArticleNum();
            int newNum = (maxNum == null ? 1 : maxNum + 1);
            article.setArticleNum(newNum);
            article.setVersion(1);  // 初始版本为 1
        } else {
            // 已有文章，查找当前最大版本并 +1
            Integer maxVersion = articleMapper.selectMaxVersionByNum(article.getArticleNum());
            article.setVersion(maxVersion == null ? 1 : maxVersion + 1);
        }

        // 将该 article_num 下的所有版本设为非当前版本
        articleMapper.updateIsCurrentToZero(article.getArticleNum());
